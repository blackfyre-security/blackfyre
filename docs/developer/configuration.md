...
| `ALLOW_UNPAID_REGISTRATION` | `"true"` permits self-service signup via `POST /api/auth/register`; `"false"` (default) returns 403 and accounts are created by invitation. Self-hosting sets `"true"`. | — |
